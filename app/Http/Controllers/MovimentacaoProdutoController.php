...
class MovimentacaoProdutoController extends Controller
{
    ...
    */
    public function relatorioPosicaoEstoque(Request $request)
    {
        ...
        // Lógica para Estoque (seleção múltipla)
        // Se o campo for um array e não estiver vazio, use whereIn
        if (is_array($request->estoque_id) && !empty($request->estoque_id)) {
            $whereData[] = ['movimentacao_produtos.estoque_id', $request->estoque_id];

            $estoques = Estoque::whereIn('id', $request->estoque_id)->pluck('estoque')->toArray();
            array_push($parametros, 'Estoques: ' . implode(', ', $estoques));
        }
        // Caso contrário, se for um único valor válido, use a lógica original
        elseif ($request->estoque_id > 0) {
            $whereData[] = ['movimentacao_produtos.estoque_id', '=', $request->estoque_id];
            array_push($parametros, 'Estoque: ' . (Estoque::find($request->estoque_id)->estoque));
        }

        // Lógica para Grupo de Produto (seleção múltipla)
        // Aplica o filtro de grupo de produto APENAS se nenhum produto específico for selecionado.
        if ((is_array($request->grupo_produto_id) && !empty($request->grupo_produto_id)) && (!is_array($request->produto_id) || empty($request->produto_id))) {
            $whereData[] = ['produtos.grupo_produto_id', $request->grupo_produto_id];

            $grupos = GrupoProduto::whereIn('id', $request->grupo_produto_id)->pluck('grupo_produto')->toArray();
            array_push($parametros, 'Grupos de Produto: ' . implode(', ', $grupos));
        }
        // Lógica para Produto (seleção múltipla)
        // Se o campo for um array e não estiver vazio, use whereIn
        elseif (is_array($request->produto_id) && !empty($request->produto_id)) {
            $whereData[] = ['produtos.id', $request->produto_id];

            $produtosNomes = Produto::whereIn('id', $request->produto_id)->pluck('produto_descricao')->toArray();
           // array_push($parametros, 'Produtos: ' . implode(', ', $produtosNomes));
        }
        // Caso contrário, se for um único valor válido, use a lógica original
        elseif ($request->produto_id > 0) {
            $whereData[] = ['produtos.id', '=', $request->produto_id];
            array_push($parametros, 'Produto: ' . (Produto::find($request->produto_id)->produto_descricao));
        }

        // Constrói a query de forma mais segura e flexível
        $produtos = DB::table('movimentacao_produtos')
            ->select(
                'produtos.id',
                'produtos.produto_descricao',
                DB::raw(
                    'SUM(
                        CASE tipo_movimentacao_produtos.eh_entrada
                            WHEN 1 THEN movimentacao_produtos.quantidade_movimentada
                            WHEN 0 THEN movimentacao_produtos.quantidade_movimentada * -1
                        END
                    ) as posicao'
                ),
                'estoques.estoque',
        ...
        foreach ($whereData as $where) {
            // Se a segunda parte da condição for um array, usa whereIn
            if (is_array($where[1])) {
                $produtos->whereIn($where[0], $where[1]);
            }
            // Caso contrário, usa where simples
            else {
                $produtos->where($where[0], $where[1], $where[2]);
            }
        }
